...
| **Bayesian Multifactor ANOVA** (Kruschke, 2015; extended models) | ≥ 2 factors (≥ 2 levels each) | One dataset with multiple categorical factors | Continuous outcomes nested by factor combinations (cells); optionally includes interaction effects | Robust Student-t likelihood for observations; Normal priors on main effects and interactions; hyperpriors on grand mean/variance; no assumption of homoscedasticity or normality required | Provides posterior distributions for main effects, interactions, and cell means, as well as variance explained (η²) per factor. Delivers intuitive quantities such as P(μᵢ > μⱼ), ROPE probabilities, and credible intervals. Enables joint assessment of main effects and interactions within a hierarchical, robust Bayesian framework. |
