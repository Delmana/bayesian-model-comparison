...
class BayesianOneWayANOVA(AbstractBayesian):
    def __init__(
        ...
        super().__init__(stan_file="bayesian_one-way_anova.stan", rope=rope, seed=seed)
    ...
    def _posterior_predictive_check(
        ...
        with warnings.catch_warnings():
            ...
            if not hasattr(self.inf_data, "posterior_predictive"):
                ...
                # safe assign (no new group creation)
    ...
    def analyse(
        ...
        **kwargs,
        ...
        """
        Summary, η², pairwise ROPE probabilities (Δμ) and a unified multi-panel posterior plot
        in the same style as BayesianIndependentTTest.
        """
        # --- PPC ---
        ...
        # --- Put eta_sq into InferenceData without creating 'posterior' anew ---
        chains = self.chains
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", message="the default dims 'chain' and 'draw' will be added automatically")
            # assign into existing posterior group
        ...
        # --- Pairwise ROPE probabilities for Δμ = μ[i] − μ[j] ---
        ...
        # -------------------- unified plotting (like independent_t_test) --------------------
        # Optional compact layout flag (same semantics as in t-test)
        plt_imp_param = False
        ...
        if plot:
            panels = []  # (title, samples, rope, within_rope, mean_value, legend?, x_label?, y_label?)
            rto = round_to

            # 1) Eta-squared η²
            eta_mean = float(np.mean(eta_sq_draws))
            panels.append((
                r"Posterior Distribution of $\eta^2$",
                eta_sq_draws,
                None,   # no ROPE for eta^2
                None,
                round(eta_mean, rto),
                False,  # legend off here
                False,  # x label
                True    # y label
            ))
            ...
            for j, lbl in enumerate(self.labels):
                mj = mu_draws[:, j]
                mj_mean = float(np.mean(mj))
                panels.append((
                    rf"Posterior Distribution of $\mu_{{{lbl}}}$",
                    mj,
                    None,
                    None,
                    round(mj_mean, rto),
            ...
            # 3) Pairwise differences Δμ with ROPE
            if self.rope is not None and self.K >= 2:
                lo, hi = self.rope
                for i in range(self.K):
                    for j in range(i + 1, self.K):
                        ...
                        inside = float(np.mean((d >= lo) & (d <= hi))) * 100.0
                        panels.append((
                            rf"Posterior Distribution of $\Delta\mu = \mu_{{{self.labels[i]}}} - \mu_{{{self.labels[j]}}}$",
                            d,
                            (lo, hi),
                            inside,
                            round(d_mean, rto),
            ...
            # 4) Optional scale/df parameters (if present in Stan fit)
            try:
                sigma_y = self._fit.stan_variable("sigma_y")
                sigma_y_mean = float(np.mean(sigma_y))
                panels.append((
                    r"Posterior Distribution of $\sigma_y$",
                    sigma_y,
                    None,
                    None,
                    round(sigma_y_mean, rto),
                    False,
                    bool(plt_imp_param),
                    False
                ))
            ...
            try:
                sigma_mu = self._fit.stan_variable("sigma_mu")
                sigma_mu_mean = float(np.mean(sigma_mu))
                panels.append((
                    r"Posterior Distribution of $\sigma_\mu$",
                    sigma_mu,
                    None,
                    None,
                    round(sigma_mu_mean, rto),
                    False,
                    True,  # likely last row -> x label on
                    True
                ))
            except Exception:
                pass
            ...
            try:
                ...
                panels.append((
                    r"Posterior Distribution of $\nu$",
                    nu,
                    None,
                    None,
                    round(nu_mean, rto),
                    True,   # show legend here
                    True,
                    False
                ))
            except Exception:
            ...
            # Layout: 2 columns like t-test, dynamic rows
            ncols = 2
            n = len(panels)
            ...
            if save:
                ...
                plt.tight_layout()
                plt.show()
        # ------------------ end unified plotting ------------------

        # --- Additional eta_sq single-panel (optional) ---
        # Kept for parity with other modules; harmless if duplicated visually in the grid above.
        if plot:
            try:
                plot_posterior_pdf(
                    inf_data=self.inf_data,
