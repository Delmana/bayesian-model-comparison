...
transformed data {
  int n_modes = 2;  // Number of modes in the mixture model

  // 90% of (exponentially distributed) means to lie in [0, max(y)].
  real lambda_mean1 = - log(1 - 0.9) / max(y1);
  real lambda_mean2 = - log(1 - 0.9) / max(y2);

  real mean_weight = 0.5;  // Mean weight for the beta distribution
  real var_weight = square(0.1);  // Variance weight for the beta distribution

  // Parameters for the beta distribution based on the mean and variance weights
  real alpha_weight =
  ...
  // Minimum mean to consider for each component, a hundredth of the minimum of the data
  real min_mean1 = min(y1) / 100;
  real min_mean2 = min(y2) / 100;
}

parameters {
  // Weights for the mixture components, constrained between 0 and 1
  real<lower=0, upper=1> weight1;
  real<lower=0, upper=1> weight2;

  // Unshifted mean parameters for the Gamma distributions, ordered positively
  ...
  vector<lower=(mean1_unshifted + min_mean1) / var_y1_upper, upper=(mean1_unshifted + min_mean1) / var_y1_lower>[n_modes] beta1;
  vector<lower=(mean2_unshifted + min_mean2) / var_y2_upper, upper=(mean2_unshifted + min_mean2) / var_y2_lower>[n_modes] beta2;
}
...
transformed parameters {
  ...
  // Alpha parameters for the Gamma distributions (elementwise product)
  vector<lower=0>[n_modes] alpha1 = mean1 .* beta1;
  vector<lower=0>[n_modes] alpha2 = mean2 .* beta2;
}

model {
  // Priors for the unshifted mean parameters, using exponential distributions
  mean1_unshifted ~ exponential(lambda_mean1);
  mean2_unshifted ~ exponential(lambda_mean2);

  // Priors for the weights, using beta distributions
  weight1 ~ beta(alpha_weight, beta_weight);
  weight2 ~ beta(alpha_weight, beta_weight);

  // Priors for the beta parameters, using uniform distributions constrained by variance bounds
  beta1 ~ uniform(mean1 / var_y1_upper, mean1 / var_y1_lower);
  beta2 ~ uniform(mean2 / var_y2_upper, mean2 / var_y2_lower);

  // Likelihoods for the data, using log mixtures of Gamma distributions
  for (n in 1:n_runs1) {
    target +=
      log_mix(weight1,
